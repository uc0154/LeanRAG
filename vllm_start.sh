...
for ((i=0;i<2;i++)); do
  (
    ...
    nohup vllm serve /cpfs04/user/zhangyaoze/vllm_weight/Qwen3-14b --served-model-name Qwen3-14b\
      --port $((8001 + i))   --max-model-len 32768  > logs/qwen3_14b_$i.log 2>&1 &
  )
done
CUDA_VISIBLE_DEVICES=2,3 nohup  vllm serve /cpfs04/user/zhangyaoze/vllm_weight/Qwen3-32b --served-model-name Qwen3-32b --port 8003 --tensor-parallel-size 2   >q32.log 2>&1 &
# --rope-scaling '{"rope_type":"yarn","factor":4.0,"original_max_position_embeddings":32768}' --max-model-len 131072
# CUDA_VISIBLE_DEVICES=0,1 nohup  vllm serve /root/Qwen2.5 --served-model-name qwen2.5 --port 8001 --tensor-parallel-size 2 --max-model-len 65536 >logs1/qwen2.5.log  2>&1 &